 ...
 * @author HoaTran
 */
public class Pre_Filtering_BioFormat_proteins implements PlugIn
{
    private final int NONE = 0, BFP = 1, TSAPPHIRE = 2, VENUS = 3, TOMATO = 4, KATUSHKA=5, DAPI=6;
//    String dir = null, subdir = null;
    public String save_dir = "", input_fn="";
    public void run(String arg) 
    {
        String[] label = {"*NONE*", "BFP", "TSAPPHIRE","VENUS", "TOMATO","KATUSHKA","NUCLEUS"};
//        int def = 0;
...
//        "organization applied to \nthe study of the islets of Langerhans");
        ...
        gd.addFileField("Input file", input_fn, 30);
//        gd.addChoice("Composite_Image :", titles, titles[0]);
        gd.addChoice("Marker_1 : ", label, label[1]);
        gd.addChoice("Marker_2 : ", label, label[2]);
        gd.addChoice("Marker_3 : ", label, label[3]);
        gd.addChoice("Marker_4: ", label, label[4]);
        gd.addChoice("Marker_5: ", label, label[5]);
        gd.addChoice("Marker_6: ", label, label[6]);
        gd.showDialog();
...
//        int i1Index = gd.getNextChoiceIndex();
        ...
        idx[4] = gd.getNextChoiceIndex();
        idx[5] = gd.getNextChoiceIndex();
...
//        String[] label_desc = {null, "C1-delta", "C2-beta","C3-alpha", "C4-dapi"};
        String[] label_desc = {null,  "C1-BFP", "C2-tSapphire","C3-venus", 
            "C4-tomato","C5-katushka","C6-NUC"};
//        switch (label) {
...
//        }
    ...
    private void preFiltering(String image_fn, int[] idx)
    {
        ImporterOptions options = null;
        try {
            options = new ImporterOptions();
        } catch (IOException ex) {
            Logger.getLogger(Pre_Filtering_BioFormat_proteins.class.getName()).log(Level.SEVERE, null, ex);
        }
        options.setId(image_fn);
...
//                IJ.run(channels[i], "Median 3D...", "x=4 y=4 z=2");
                IJ.log(channels[i].getTitle());
                IJ.log("Median pre-filtering image... ");
                IJ.run(channels[i], "Median...", "radius=2");
