...
cell_type_stat <- function(nodes_df, meta_clones, save_dir, datatag){
  ## Summary number of each cell type
  nodes_stat <- nodes_df %>% 
  ...
  meta_clones$clone_id <- gsub('_','',meta_clones$clone_id)

  # nodes_stat <- nodes_stat %>% left_join(meta_clones, by=c('cluster_label'='clone_id'))
  nodes_stat <- meta_clones %>% left_join(nodes_stat, by=c('clone_id'='cluster_label'))
...
viz_wholetissue_v2 <- function(nodes_df, save_dir, datatag, meta_clones_fn, 
                               col=NULL, invert_coord=T, marker_intensity_viz=F, xmax=NULL, ymax=NULL, marker_present_absent_viz=T){
  ...
  if(dim(nodes_df)[1]<10000){
    ...
    cell_size <- 0.001  # change the size of dots if you want bigger dot size in visualization
    ht <- round(ymax/10,0)
    wd <- round(xmax/10,0)
  }
  print(unique(nodes_df$cluster_label))
  
  ## Visualizing all cell types in data
  p <- ggplot(nodes_df, aes(x = x, y = y)) + 
    ...
    theme(legend.position = "none",
          ...
          panel.background = element_rect(fill = "black",
                                          colour = "black"))
  # p
  png(paste0(save_dir,datatag,'_celltype_clonal_labels.png'), 
  ...
  # nodes_stat$number_of_cells <- ifelse(nodes_stat$number_of_cells==0, 0.01, nodes_stat$number_of_cells)
  ...
  # p1
  png(paste0(save_dir,datatag, '_cell_types_summary.png'), 
      height = 2*850, width=2*850,res = 2*72)
  print(p1)
  dev.off()
  meta_clones$clone_id <- gsub('_','',meta_clones$clone_id)
  nodes_stat$clone_info <- NULL
  nodes_stat$number_of_cells <- NULL
  
  meta_clones1 <- meta_clones %>% left_join(nodes_stat, by=c('clone_id'='cluster_label'))
  nodes_stat <- nodes_stat %>% left_join(meta_clones, by=c('cluster_label'='clone_id'))
  # nodes_stat$clone_desc <- paste0(nodes_stat$cluster_label, ' (',nodes_stat$nb_cells,'): ',nodes_stat$clone_desc)
  data.table::fwrite(meta_clones1, paste0(save_dir,datatag, '_cell_types_summary.csv'))
  ...
  plg <- ggplot(nodes_stat, aes(x=factor(cluster_label, levels = cls), 
    ...
    theme(axis.text.x = element_text(size=9, angle=90),
          ...
          legend.text = element_text(size=9)) + 
    labs(x='Clone Id', y= 'log10 (# cells)', title=paste0('Summary cell type in - ',datatag))
  png(paste0(save_dir,datatag, '_cell_types_summary_clone_labels.png'), 
      height = 2*850, width=2*1400,res = 2*72)
  print(plg)
  dev.off()
  # plg11 <- cowplot::ggdraw() + cowplot::draw_plot(cowplot::get_legend(plg))
  
  if(marker_present_absent_viz){
    ls_markers <- c('C1_BFP','C2_tSapphire','C3_venus','C4_tomato','C5_katushka')
    marker_cols_use <- c("C1_BFP"="blue", "C2_tSapphire"="green","C3_venus"="yellow","C4_tomato"="red", "C5_katushka"="purple")
    for(m in ls_markers){
      plt_var <- paste0(m,'_status')
      nodes_filtered_df <- nodes_df %>%
        dplyr::filter(!!sym(plt_var)==1)
        
      # sum(nodes_df1$C1_BFP_mean_corerage>0)
      p <- ggplot(nodes_df, aes(x = x, y = y)) + 
        geom_point(color = '#C0C0C0', size=cell_size) +   # drawing all cells in the tissue area with gray colors
        geom_point(data=nodes_filtered_df, color=marker_cols_use[m], size=cell_size) + 
        # geom_text(aes(label=celltype_desc))+
        # annotate('text', x = genes$xcoord, y = -log10(genes$ycoord), label = genes$celltype_desc)+
        # scale_color_viridis(option = "D")+
        theme_bw(base_size = 12) + 
      ...
      # p
      png(paste0(save_dir,stringr::str_sub(m, 4, nchar(m)),'_present_absent_debug_plot.png'), 
          height = 2*ht, width=2*wd,res = 2*72)
      print(p)
      dev.off()
    }
  ...
  print(paste0("Store visualization figures into folder: ",save_dir,datatag))
...
get_celltype_v4 <- function(nodes_df, edges_df, 
                            save_dir, datatag, 
                            celltype_by_pct=T, save_data=T, thres_vol_marker=15, # 15% covered by a given marker 
  ...
  if(save_data){
    output_nodes_fn <- paste0(save_dir, datatag,'_nodes_celltype_threshold_marker_',thres_vol_marker,'.csv')
    data.table::fwrite(nodes_df, output_nodes_fn)
    print(paste0('Store output of cell type labels as ', output_nodes_fn))
    if(!is.null(edges_df)){
      ...
      edges_df <- edges_df %>%
        dplyr::filter(from %in% nodes_df$cell_id & to %in% nodes_df$cell_id)
      print(dim(edges_df))
      data.table::fwrite(edges_df, paste0(save_dir, datatag,'_edges_celltype_threshold_marker_',thres_vol_marker,'.csv.gz'))
    }
  }
  res <- list(nodes=nodes_df, edges=edges_df)
  return(res)
...
get_color_clone <- function(clones){
  col <- c("eBFP2"="#fbeb14",                                      
           ...
           "tSapphire, Venus, Katushka"="#e57617",                 
