...
viz_wholetissue <- function(save_dir, datatag, xmax=NULL, ymax=NULL, 
  ...
  nodes_stat$cluster_label <- factor(cluster_label, levels = nodes_stat$cluster_label)
  ...
  p1 <- ggplot(nodes_stat, aes(x=factor(cluster_label, levels = cls), 
    ...
    labs(x='Clone Id', y= '# cells', title='# cells in lung Ms876')
  # p1
  ...
  data.table::fwrite(nodes_stat, paste0(save_dir,datatag, 'cell_types_summary.csv'))
